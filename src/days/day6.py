...
In this visual representation, when two objects are
connected by a line, the one on the right directly orbits the one on the left.
...
class Day6(Day):
    ...
    def _get_total_orbits_for(self, mass, masses, orbits = 0):
        parent = masses.get(mass, None)
        if parent is not None:
            orbits = self._get_total_orbits_for(parent, masses, orbits + 1)
        
        return orbits

    def _get_parents(self, mass, masses):
        parents = []
        parent = masses.get(mass, None)
        while parent is not None:
            parents.append(parent)
            parent = masses.get(parent, None)
    ...
    def _puzzle1(self):
        ...
        for line in self._input_stream:
            ...
            orbit = line.strip()
            parent, mass = orbit.split(")")
            masses[mass] = parent

        orbit = 0
        for mass in masses:
            orbit += self._get_total_orbits_for(mass, masses)
    ...
    def _puzzle2(self):
        ...
        SANparents = self._get_parents("SAN", masses)
        YOUparents = self._get_parents("YOU", masses)

        for sidx, parent in enumerate(SANparents):
            if parent in YOUparents:
                yidx = YOUparents.index(parent)
                answer = str(sidx + yidx)
                break
