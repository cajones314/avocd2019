...
class Day2(Day):
    def _run_intcode_program(self, program: list, noun: int=None, verb: int=None):
        memory = []
    ...
    def _read_program(self) -> (str, list):
    ...
    def _puzzle1(self):
        ...
        program_name, program = self._read_program()

        # per context rules change these memory positions
        if program_name == "1202 program alarm":
            result = self._run_intcode_program(program, 12, 2)
            result = [ result[0] ]
        else:
    ...
    def _puzzle2(self):
        answer = ''
        _, program = self._read_program()

        # itertools product to the rescue for double loops
        for noun, verb in product(range(0,100), range(0,100)):
            result = self._run_intcode_program(program.copy(), noun, verb)
            if result[0] == 19690720:
                answer = f"{100*noun+verb}"
                break
